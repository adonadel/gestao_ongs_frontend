...
const UserUpdate: React.FC = () => {
    ...
    const authValue = JSON.parse(storedValue || '{}');
    ...
    useEffect(() => {
        if (!authValue?.state.userData.role.permissions.filter(permission => permission.name === "user-update").length > 0) {
    ...
    return (
        <>
            <Typography variant="h3" fontSize={'2rem'} marginTop='20px' fontWeight={'medium'}>{isEditMode ? 'Editar' : 'Criar'} usuário</Typography>
